...
#ematest IS_LANG            
            self.is_lang.setChecked(c['display'].get('is_lang', False))
#ematest IS_LANG            
    ...
    def setup_ui(self):  # {{{
        ...
        h.addWidget(ins)
#ematest IS_LANG
#        self.is_names.toggled.connect(self.datatype_changed)
#ematest IS_LANG
        from functools import partial
        self.is_names.clicked.connect(partial(self.datatype_changed,self.is_names))
        self.is_lang = ins = QCheckBox(_("Languages"), self)
        ins.setToolTip('<p>' + _('Check this box if you want this column used to store langages,'
             'can be used for books original lamguahes in case of translated books')+'</p>')
        h.addWidget(ins)
        self.is_lang.clicked.connect(partial(self.datatype_changed,self.is_lang))
#ematest IS_LANG
...
#ematest IS_LANG
        self.is_lang.setVisible(col_type == '*text')
        self.is_names.setVisible(col_type == '*text')
        is_names = self.is_names.isChecked()
        is_lang = self.is_lang.isChecked()
        if len(args) >= 2:
            sender=args[0]
            check=args[1]
            if check == False:
                self.is_names.setChecked(False)
                self.is_lang.setChecked(False)
            elif sender==self.is_lang:
                self.is_names.setChecked(False)
                self.is_lang.setChecked(True)
            else:
...
#ematest IS_LANG            
            display_dict = {'is_lang': self.is_lang.isChecked(),'is_names': self.is_names.isChecked()}
